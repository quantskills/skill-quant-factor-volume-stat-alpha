---
name: quant-real-factor-10d-volatility-scaled-volume-ts-rank
description: Use when computing the 10D Volatility Scaled Volume TS Rank factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Volatility Scaled Volume TS Rank

Use this Skill to compute `10日波动缩放成交量时序排名` / `10D Volatility Scaled Volume TS Rank` from caller-provided OHLCV data.
