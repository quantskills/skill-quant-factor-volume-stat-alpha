---
name: quant-real-factor-5d-stability-scaled-volume-ts-rank
description: Use when computing the 5D Stability Scaled Volume TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Volume TS Rank

Use this Skill to compute `5日稳定度缩放成交量时序排名` / `5D Stability Scaled Volume TS Rank` from caller-provided OHLCV data.
