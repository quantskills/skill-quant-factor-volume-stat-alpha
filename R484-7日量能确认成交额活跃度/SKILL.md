---
name: quant-real-factor-7d-volume-confirmed-dollar-volume-activity
description: Use when computing the 7D Volume Confirmed Dollar Volume Activity factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Dollar Volume Activity

Use this Skill to compute `7日量能确认成交额活跃度` / `7D Volume Confirmed Dollar Volume Activity` from caller-provided OHLCV data.
