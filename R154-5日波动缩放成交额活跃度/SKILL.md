---
name: quant-real-factor-5d-volatility-scaled-dollar-volume-activity
description: Use when computing the 5D Volatility Scaled Dollar Volume Activity factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Dollar Volume Activity

Use this Skill to compute `5日波动缩放成交额活跃度` / `5D Volatility Scaled Dollar Volume Activity` from caller-provided OHLCV data.
