---
name: quant-real-factor-5d-smoothed-volume-z-score
description: Use when computing the 5D Smoothed Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Volume Z-Score

Use this Skill to compute `5日平滑成交量标准分` / `5D Smoothed Volume Z-Score` from caller-provided OHLCV data.
