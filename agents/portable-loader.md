...
```text
You have access to a local skill named quant-factor-volume-stat-alpha at:
<QUANT_FACTOR_VOLUME_STAT_ALPHA_SKILL_ROOT>

When the user request matches this skill's SKILL.md description:
1. Read <QUANT_FACTOR_VOLUME_STAT_ALPHA_SKILL_ROOT>/SKILL.md.
2. Follow the workflow and guardrails in that file exactly.
3. Load referenced files under <QUANT_FACTOR_VOLUME_STAT_ALPHA_SKILL_ROOT>/references/ only when needed.
