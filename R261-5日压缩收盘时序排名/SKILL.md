---
name: quant-real-factor-5d-compressed-close-ts-rank
description: Use when computing the 5D Compressed Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Compressed Close TS Rank

Use this Skill to compute `5日压缩收盘时序排名` / `5D Compressed Close TS Rank` from caller-provided OHLCV data.
