---
name: quant-real-factor-7d-time-series-ranked-close-ts-rank
description: Use when computing the 7D Time-Series Ranked Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Close TS Rank

Use this Skill to compute `7日时序排名收盘时序排名` / `7D Time-Series Ranked Close TS Rank` from caller-provided OHLCV data.
