---
name: quant-real-factor-7d-smoothed-volume-z-score
description: Use when computing the 7D Smoothed Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Volume Z-Score

Use this Skill to compute `7日平滑成交量标准分` / `7D Smoothed Volume Z-Score` from caller-provided OHLCV data.
