---
name: quant-real-factor-5d-stability-scaled-close-ts-rank
description: Use when computing the 5D Stability Scaled Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Close TS Rank

Use this Skill to compute `5日稳定度缩放收盘时序排名` / `5D Stability Scaled Close TS Rank` from caller-provided OHLCV data.
