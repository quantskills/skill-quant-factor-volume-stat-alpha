...
`10日压缩收益偏度` 是一个框架中立的 OHLCV 因子 Skill，属于 `分布` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(ret_skew, window=10)
```
...
`10D Compressed Return Skewness` is a framework-neutral OHLCV factor Skill in the `Distribution` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9925`
- 5-day Rank IC mean: `-0.005143`
- 5-day ICIR: `-0.2608`
- Quintile Q5-Q1 mean: `0.001061`
- Top-quintile turnover: `0.2821`
