---
name: quant-real-factor-5d-stability-scaled-volume-z-score
description: Use when computing the 5D Stability Scaled Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Volume Z-Score

Use this Skill to compute `5日稳定度缩放成交量标准分` / `5D Stability Scaled Volume Z-Score` from caller-provided OHLCV data.
