---
name: quant-real-factor-5d-z-scored-close-ts-rank
description: Use when computing the 5D Z-Scored Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Close TS Rank

Use this Skill to compute `5日标准化收盘时序排名` / `5D Z-Scored Close TS Rank` from caller-provided OHLCV data.
