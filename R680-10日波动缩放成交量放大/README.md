...
`10日波动缩放成交量放大` 是一个框架中立的 OHLCV 因子 Skill，属于 `量能` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
vol_scaled(volume_ratio, window=10)
```

该因子把“成交量放大”与“波动缩放”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
`10D Volatility Scaled Volume Expansion` is a framework-neutral OHLCV factor Skill in the `Volume` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Volume Expansion with a Volatility Scaled transform to test cross-sectional sortability in price, volume, or volatility states.
