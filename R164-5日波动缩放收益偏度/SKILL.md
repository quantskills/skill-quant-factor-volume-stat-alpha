---
name: quant-real-factor-5d-volatility-scaled-return-skewness
description: Use when computing the 5D Volatility Scaled Return Skewness factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Return Skewness

Use this Skill to compute `5日波动缩放收益偏度` / `5D Volatility Scaled Return Skewness` from caller-provided OHLCV data.
