...
`10日稳定度缩放收盘时序排名` 是一个框架中立的 OHLCV 因子 Skill，属于 `排序` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
stability(ts_rank_close, window=10)
```

该因子把“收盘时序排名”与“稳定度缩放”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
`10D Stability Scaled Close TS Rank` is a framework-neutral OHLCV factor Skill in the `Time Series Rank` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Close TS Rank with a Stability Scaled transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9902`
- 5-day Rank IC mean: `0.006842`
- 5-day ICIR: `0.2512`
- Quintile Q5-Q1 mean: `0.001224`
- Top-quintile turnover: `0.4347`
