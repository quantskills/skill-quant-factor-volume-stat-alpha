---
name: quant-real-factor-10d-volatility-scaled-close-ts-rank
description: Use when computing the 10D Volatility Scaled Close TS Rank factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Volatility Scaled Close TS Rank

Use this Skill to compute `10日波动缩放收盘时序排名` / `10D Volatility Scaled Close TS Rank` from caller-provided OHLCV data.
