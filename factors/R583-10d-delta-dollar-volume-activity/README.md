...
`10日变化成交额活跃度` 是一个框架中立的 OHLCV 因子 Skill，属于 `流动性` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
delta(dollar_volume, window=10)
```

该因子把“成交额活跃度”与“变化”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9917`
- 5日 Rank IC 均值: `0.003339`
- 5日 ICIR: `0.1430`
- 五分组 Q5-Q1 均值: `0.000119`
- Top 组换手: `0.7033`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Delta Dollar Volume Activity` is a framework-neutral OHLCV factor Skill in the `Liquidity` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Dollar Volume Activity with a Delta transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9917`
- 5-day Rank IC mean: `0.003339`
- 5-day ICIR: `0.1430`
- Quintile Q5-Q1 mean: `0.000119`
- Top-quintile turnover: `0.7033`
