---
name: quant-real-factor-5d-time-series-ranked-volume-ts-rank
description: Use when computing the 5D Time-Series Ranked Volume TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Volume TS Rank

Use this Skill to compute `5日时序排名成交量时序排名` / `5D Time-Series Ranked Volume TS Rank` from caller-provided OHLCV data.
