---
name: quant-real-factor-7d-time-series-ranked-return-skewness
description: Use when computing the 7D Time-Series Ranked Return Skewness factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Return Skewness

Use this Skill to compute `7日时序排名收益偏度` / `7D Time-Series Ranked Return Skewness` from caller-provided OHLCV data.
