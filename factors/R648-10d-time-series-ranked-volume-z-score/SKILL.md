---
name: quant-real-factor-10d-time-series-ranked-volume-z-score
description: Use when computing the 10D Time-Series Ranked Volume Z-Score factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Time-Series Ranked Volume Z-Score

Use this Skill to compute `10日时序排名成交量标准分` / `10D Time-Series Ranked Volume Z-Score` from caller-provided OHLCV data.
