...
`10日时序排名成交量标准分` 是一个框架中立的 OHLCV 因子 Skill，属于 `量能` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
rank(volume_z, window=10)
```

该因子把“成交量标准分”与“时序排名”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9902`
- 5日 Rank IC 均值: `0.001277`
- 5日 ICIR: `0.0570`
- 五分组 Q5-Q1 均值: `0.000610`
- Top 组换手: `0.6567`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Time-Series Ranked Volume Z-Score` is a framework-neutral OHLCV factor Skill in the `Volume` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Volume Z-Score with a Time-Series Ranked transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9902`
- 5-day Rank IC mean: `0.001277`
- 5-day ICIR: `0.0570`
- Quintile Q5-Q1 mean: `0.000610`
- Top-quintile turnover: `0.6567`
