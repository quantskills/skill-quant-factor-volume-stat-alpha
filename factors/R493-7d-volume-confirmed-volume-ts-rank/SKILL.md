---
name: quant-real-factor-7d-volume-confirmed-volume-ts-rank
description: Use when computing the 7D Volume Confirmed Volume TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Volume TS Rank

Use this Skill to compute `7日量能确认成交量时序排名` / `7D Volume Confirmed Volume TS Rank` from caller-provided OHLCV data.
