---
name: quant-real-factor-7d-time-series-ranked-return-kurtosis
description: Use when computing the 7D Time-Series Ranked Return Kurtosis factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Return Kurtosis

Use this Skill to compute `7日时序排名收益峰度` / `7D Time-Series Ranked Return Kurtosis` from caller-provided OHLCV data.
