---
name: quant-real-factor-7d-volatility-scaled-return-skewness
description: Use when computing the 7D Volatility Scaled Return Skewness factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Return Skewness

Use this Skill to compute `7日波动缩放收益偏度` / `7D Volatility Scaled Return Skewness` from caller-provided OHLCV data.
