---
name: quant-real-factor-5d-z-scored-return-kurtosis
description: Use when computing the 5D Z-Scored Return Kurtosis factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Return Kurtosis

Use this Skill to compute `5日标准化收益峰度` / `5D Z-Scored Return Kurtosis` from caller-provided OHLCV data.
