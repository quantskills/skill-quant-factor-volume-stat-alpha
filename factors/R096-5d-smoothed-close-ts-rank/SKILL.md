---
name: quant-real-factor-5d-smoothed-close-ts-rank
description: Use when computing the 5D Smoothed Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Close TS Rank

Use this Skill to compute `5日平滑收盘时序排名` / `5D Smoothed Close TS Rank` from caller-provided OHLCV data.
