---
name: quant-real-factor-5d-time-series-ranked-dollar-volume-activity
description: Use when computing the 5D Time-Series Ranked Dollar Volume Activity factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Dollar Volume Activity

Use this Skill to compute `5日时序排名成交额活跃度` / `5D Time-Series Ranked Dollar Volume Activity` from caller-provided OHLCV data.
