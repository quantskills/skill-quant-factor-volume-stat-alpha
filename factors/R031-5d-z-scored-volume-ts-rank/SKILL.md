---
name: quant-real-factor-5d-z-scored-volume-ts-rank
description: Use when computing the 5D Z-Scored Volume TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Volume TS Rank

Use this Skill to compute `5日标准化成交量时序排名` / `5D Z-Scored Volume TS Rank` from caller-provided OHLCV data.
