---
name: quant-real-factor-7d-z-scored-price-volume-correlation
description: Use when computing the 7D Z-Scored Price Volume Correlation factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Z-Scored Price Volume Correlation

Use this Skill to compute `7日标准化量价相关` / `7D Z-Scored Price Volume Correlation` from caller-provided OHLCV data.
