---
name: quant-real-factor-7d-time-series-ranked-obv-slope
description: Use when computing the 7D Time-Series Ranked OBV Slope factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked OBV Slope

Use this Skill to compute `7日时序排名OBV斜率` / `7D Time-Series Ranked OBV Slope` from caller-provided OHLCV data.
