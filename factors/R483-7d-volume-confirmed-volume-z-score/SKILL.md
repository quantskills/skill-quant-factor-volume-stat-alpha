---
name: quant-real-factor-7d-volume-confirmed-volume-z-score
description: Use when computing the 7D Volume Confirmed Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Volume Z-Score

Use this Skill to compute `7日量能确认成交量标准分` / `7D Volume Confirmed Volume Z-Score` from caller-provided OHLCV data.
