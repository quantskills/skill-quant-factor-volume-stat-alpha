---
name: quant-real-factor-5d-stability-scaled-return-kurtosis
description: Use when computing the 5D Stability Scaled Return Kurtosis factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Return Kurtosis

Use this Skill to compute `5日稳定度缩放收益峰度` / `5D Stability Scaled Return Kurtosis` from caller-provided OHLCV data.
