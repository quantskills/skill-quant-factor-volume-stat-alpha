---
name: quant-real-factor-5d-volatility-scaled-return-kurtosis
description: Use when computing the 5D Volatility Scaled Return Kurtosis factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Return Kurtosis

Use this Skill to compute `5日波动缩放收益峰度` / `5D Volatility Scaled Return Kurtosis` from caller-provided OHLCV data.
