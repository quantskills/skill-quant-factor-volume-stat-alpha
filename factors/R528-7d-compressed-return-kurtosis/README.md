...
`7日压缩收益峰度` 是一个框架中立的 OHLCV 因子 Skill，属于 `分布` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(ret_kurt, window=7)
```

该因子把“收益峰度”与“压缩”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9932`
- 5日 Rank IC 均值: `0.001101`
- 5日 ICIR: `0.0660`
- 五分组 Q5-Q1 均值: `0.000042`
- Top 组换手: `0.4093`
- 无未来函数检查: `True`
- 状态: `pass`
...
`7D Compressed Return Kurtosis` is a framework-neutral OHLCV factor Skill in the `Distribution` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Return Kurtosis with a Compressed transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9932`
- 5-day Rank IC mean: `0.001101`
- 5-day ICIR: `0.0660`
- Quintile Q5-Q1 mean: `0.000042`
- Top-quintile turnover: `0.4093`
