---
name: quant-real-factor-5d-volume-confirmed-volume-z-score
description: Use when computing the 5D Volume Confirmed Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Volume Z-Score

Use this Skill to compute `5日量能确认成交量标准分` / `5D Volume Confirmed Volume Z-Score` from caller-provided OHLCV data.
