---
name: quant-real-factor-7d-volatility-scaled-volume-expansion
description: Use when computing the 7D Volatility Scaled Volume Expansion factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Volume Expansion

Use this Skill to compute `7日波动缩放成交量放大` / `7D Volatility Scaled Volume Expansion` from caller-provided OHLCV data.
