---
name: quant-real-factor-7d-volume-confirmed-close-ts-rank
description: Use when computing the 7D Volume Confirmed Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Close TS Rank

Use this Skill to compute `7日量能确认收盘时序排名` / `7D Volume Confirmed Close TS Rank` from caller-provided OHLCV data.
