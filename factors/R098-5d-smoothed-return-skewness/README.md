...
`5日平滑收益偏度` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `分布` 类。

### 因子逻辑

```text
smooth(ret_skew, window=5)
```
...
`5D Smoothed Return Skewness` is a real-data-validated framework-neutral OHLCV factor Skill in the `Distribution` family.
