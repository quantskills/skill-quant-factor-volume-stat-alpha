---
name: quant-real-factor-7d-time-series-ranked-price-volume-correlation
description: Use when computing the 7D Time-Series Ranked Price Volume Correlation factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Price Volume Correlation

Use this Skill to compute `7日时序排名量价相关` / `7D Time-Series Ranked Price Volume Correlation` from caller-provided OHLCV data.
