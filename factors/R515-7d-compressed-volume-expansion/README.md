...
`7日压缩成交量放大` 是一个框架中立的 OHLCV 因子 Skill，属于 `量能` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(volume_ratio, window=7)
```
...
`7D Compressed Volume Expansion` is a framework-neutral OHLCV factor Skill in the `Volume` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9947`
- 5-day Rank IC mean: `0.005896`
- 5-day ICIR: `0.2484`
- Quintile Q5-Q1 mean: `0.001228`
- Top-quintile turnover: `0.6642`
