---
name: quant-real-factor-5d-time-series-ranked-close-ts-rank
description: Use when computing the 5D Time-Series Ranked Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Close TS Rank

Use this Skill to compute `5日时序排名收盘时序排名` / `5D Time-Series Ranked Close TS Rank` from caller-provided OHLCV data.
