---
name: quant-real-factor-5d-time-series-ranked-return-skewness
description: Use when computing the 5D Time-Series Ranked Return Skewness factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Return Skewness

Use this Skill to compute `5日时序排名收益偏度` / `5D Time-Series Ranked Return Skewness` from caller-provided OHLCV data.
