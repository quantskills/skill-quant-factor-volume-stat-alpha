---
name: quant-real-factor-7d-smoothed-close-ts-rank
description: Use when computing the 7D Smoothed Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Close TS Rank

Use this Skill to compute `7日平滑收盘时序排名` / `7D Smoothed Close TS Rank` from caller-provided OHLCV data.
