---
name: quant-real-factor-7d-smoothed-dollar-volume-activity
description: Use when computing the 7D Smoothed Dollar Volume Activity factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Dollar Volume Activity

Use this Skill to compute `7日平滑成交额活跃度` / `7D Smoothed Dollar Volume Activity` from caller-provided OHLCV data.
