---
name: quant-real-factor-7d-volatility-scaled-return-kurtosis
description: Use when computing the 7D Volatility Scaled Return Kurtosis factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Return Kurtosis

Use this Skill to compute `7日波动缩放收益峰度` / `7D Volatility Scaled Return Kurtosis` from caller-provided OHLCV data.
