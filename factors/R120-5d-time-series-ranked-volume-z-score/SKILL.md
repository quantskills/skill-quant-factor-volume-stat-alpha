---
name: quant-real-factor-5d-time-series-ranked-volume-z-score
description: Use when computing the 5D Time-Series Ranked Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Volume Z-Score

Use this Skill to compute `5日时序排名成交量标准分` / `5D Time-Series Ranked Volume Z-Score` from caller-provided OHLCV data.
