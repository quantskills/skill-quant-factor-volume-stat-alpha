---
name: quant-real-factor-7d-z-scored-close-ts-rank
description: Use when computing the 7D Z-Scored Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Z-Scored Close TS Rank

Use this Skill to compute `7日标准化收盘时序排名` / `7D Z-Scored Close TS Rank` from caller-provided OHLCV data.
