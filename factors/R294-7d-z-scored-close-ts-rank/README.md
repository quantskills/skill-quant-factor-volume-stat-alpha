...
`7日标准化收盘时序排名` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `排序` 类。

### 因子逻辑

```text
z(ts_rank_close, window=7)
```
...
`7D Z-Scored Close TS Rank` is a real-data-validated framework-neutral OHLCV factor Skill in the `Time Series Rank` family.
