---
name: quant-real-factor-10d-time-series-ranked-dollar-volume-activity
description: Use when computing the 10D Time-Series Ranked Dollar Volume Activity factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Time-Series Ranked Dollar Volume Activity

Use this Skill to compute `10日时序排名成交额活跃度` / `10D Time-Series Ranked Dollar Volume Activity` from caller-provided OHLCV data.
