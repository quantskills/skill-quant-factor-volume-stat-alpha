---
name: quant-real-factor-7d-stability-scaled-volume-z-score
description: Use when computing the 7D Stability Scaled Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Stability Scaled Volume Z-Score

Use this Skill to compute `7日稳定度缩放成交量标准分` / `7D Stability Scaled Volume Z-Score` from caller-provided OHLCV data.
