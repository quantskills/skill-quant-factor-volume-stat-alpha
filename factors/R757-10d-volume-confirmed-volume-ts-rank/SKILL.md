---
name: quant-real-factor-10d-volume-confirmed-volume-ts-rank
description: Use when computing the 10D Volume Confirmed Volume TS Rank factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Volume Confirmed Volume TS Rank

Use this Skill to compute `10日量能确认成交量时序排名` / `10D Volume Confirmed Volume TS Rank` from caller-provided OHLCV data.
