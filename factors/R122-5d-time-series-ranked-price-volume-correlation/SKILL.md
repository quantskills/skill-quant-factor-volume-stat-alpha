---
name: quant-real-factor-5d-time-series-ranked-price-volume-correlation
description: Use when computing the 5D Time-Series Ranked Price Volume Correlation factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Price Volume Correlation

Use this Skill to compute `5日时序排名量价相关` / `5D Time-Series Ranked Price Volume Correlation` from caller-provided OHLCV data.
