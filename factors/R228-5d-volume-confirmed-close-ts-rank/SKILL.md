---
name: quant-real-factor-5d-volume-confirmed-close-ts-rank
description: Use when computing the 5D Volume Confirmed Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Close TS Rank

Use this Skill to compute `5日量能确认收盘时序排名` / `5D Volume Confirmed Close TS Rank` from caller-provided OHLCV data.
