---
name: quant-real-factor-5d-volatility-scaled-volume-ts-rank
description: Use when computing the 5D Volatility Scaled Volume TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Volume TS Rank

Use this Skill to compute `5日波动缩放成交量时序排名` / `5D Volatility Scaled Volume TS Rank` from caller-provided OHLCV data.
