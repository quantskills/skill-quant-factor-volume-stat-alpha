---
name: quant-real-factor-7d-volatility-scaled-close-ts-rank
description: Use when computing the 7D Volatility Scaled Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Close TS Rank

Use this Skill to compute `7日波动缩放收盘时序排名` / `7D Volatility Scaled Close TS Rank` from caller-provided OHLCV data.
