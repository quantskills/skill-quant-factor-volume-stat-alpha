---
name: quant-real-factor-7d-time-series-ranked-volume-z-score
description: Use when computing the 7D Time-Series Ranked Volume Z-Score factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Volume Z-Score

Use this Skill to compute `7日时序排名成交量标准分` / `7D Time-Series Ranked Volume Z-Score` from caller-provided OHLCV data.
