---
name: quant-real-factor-7d-delta-close-ts-rank
description: Use when computing the 7D Delta Close TS Rank factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Delta Close TS Rank

Use this Skill to compute `7日变化收盘时序排名` / `7D Delta Close TS Rank` from caller-provided OHLCV data.
