---
name: quant-real-factor-5d-volume-confirmed-dollar-volume-activity
description: Use when computing the 5D Volume Confirmed Dollar Volume Activity factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Dollar Volume Activity

Use this Skill to compute `5日量能确认成交额活跃度` / `5D Volume Confirmed Dollar Volume Activity` from caller-provided OHLCV data.
