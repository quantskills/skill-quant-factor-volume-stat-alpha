---
name: quant-real-factor-5d-time-series-ranked-volume-expansion
description: Use when computing the 5D Time-Series Ranked Volume Expansion factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Volume Expansion

Use this Skill to compute `5日时序排名成交量放大` / `5D Time-Series Ranked Volume Expansion` from caller-provided OHLCV data.
