---
name: quant-real-factor-7d-time-series-ranked-volume-expansion
description: Use when computing the 7D Time-Series Ranked Volume Expansion factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Volume Expansion

Use this Skill to compute `7日时序排名成交量放大` / `7D Time-Series Ranked Volume Expansion` from caller-provided OHLCV data.
