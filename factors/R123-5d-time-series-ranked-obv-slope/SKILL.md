---
name: quant-real-factor-5d-time-series-ranked-obv-slope
description: Use when computing the 5D Time-Series Ranked OBV Slope factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked OBV Slope

Use this Skill to compute `5日时序排名OBV斜率` / `5D Time-Series Ranked OBV Slope` from caller-provided OHLCV data.
